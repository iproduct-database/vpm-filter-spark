...
class MDR(object):
    ...
    def parseHtml(self, html, encoding='utf8'):
        if isinstance(html, six.text_type):
            html = html.encode(encoding)

        parser = etree.HTMLParser(encoding=encoding, remove_comments=True, remove_blank_text=True, remove_pis=True)
        doc = etree.parse(BytesIO(html), parser)
        return doc

    def list_candidates(self, doc):
        """
        ...
        for key, elements in six.iteritems(d):
            deepest_common_ancestor = "/".join(common_prefix(*[xpath.split('/') for xpath in elements]))
            counter[deepest_common_ancestor] += 1

        return [doc.xpath(k)[0] for k,v in sorted(list(counter.items()), key=operator.itemgetter(1), reverse=True) if len(doc.xpath(k)[0]) >= 2]
...
class RecordFinder(object):
    ...
    def find_best_division(self, elements, clusters):
        ...
        for c in set(clusters):
            records = []
            for group in split_sequence(list(zip(elements, clusters)), lambda x: x[1] == c):
    ...
    def find_division(self, elements, clusters, cluster):
        ...
        for group in split_sequence(list(zip(elements, clusters)), lambda x: x[1] == cluster):
            _clusters = [g[1] for g in group]
            if len(_clusters) < len(clusters):
                records.append(Record(*[g[0] for g in group]))
